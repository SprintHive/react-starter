...
module.exports = ({globalConfig, topicConfig, streamOptions, consumerConfig} = defaultConfig) => {
  ...
  const messageReceived = (observer, message) => {
    const {timestamp, offset, topic, partition, key} = message;
    const action = JSON.parse(message.value.toString());
    action.meta = {...action.meta, ...{timestamp, offset, topic, partition, key: key.toString()}}
  ...
  return Observable.create(function (observer) {
    stream.on('data', function (message) {
      if (replay && offset) {
        message.offset > offset && messageReceived(observer, message);
      } else {
        messageReceived(observer, message);
      }
    });
